...
analyzer::analyzer(std::string_view filename) : compiler_instance() {
    ...
    clang::CompilerInvocation::setLangDefaults(lang_opts, clang::InputKind(clang::Language::OpenCL),
                                               target_info->getTriple(), compiler_instance.getPreprocessorOpts().Includes,
...
void analyzer::analyze(std::string_view kernel_name,
                       const uint32_t work_dim, const size_t* global_work_size, const size_t* local_work_size,
    ...
    const analyzer_parameters analyzer_parameters = { std::string(kernel_name), args, work_dim, std::vector(global_work_size, global_work_size + work_dim), std::vector(local_work_size, local_work_size + work_dim) };
