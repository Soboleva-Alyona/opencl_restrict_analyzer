...
namespace {
    ...
    void insert_inner_scopes(std::vector<const scope*>& vector, const scope* scope) {
        vector.push_back(scope);
        std::for_each(scope->inner_begin(), scope->inner_end(), [&](const class scope* inner) {
            insert_inner_scopes(vector, inner);
        });
    }
}

void restrict_checker::check_memory_access(const class scope* const scope, const clang::Expr* const expr,
    ...
    if (clang::isa<clang::ArraySubscriptExpr>(expr)) {
        ...
        return check_memory_access(scope, clang::cast<clang::ImplicitCastExpr>(expr)->getSubExpr(), access_type, address);
    }
    if (value_decl == nullptr) {
        return;
    }
    const auto* var = scope_ctx.get_var(value_decl);
    write_access(scope, address, var);
    ...
    if (value_decl->getType().isRestrictQualified() && access_type == WRITE) {
        write_restrict_write(scope, address, var);
        //restrict_writes[var->scope].emplace(var);

        std::vector<const class scope*> affected_scopes;
        insert_inner_scopes(affected_scopes, var->scope);

        const z3::expr other_id = z3_ctx.constant("ID", z3_ctx.int_sort());
        z3::expr condition = z3_ctx.bool_val(false);
        for (const auto* affected_scope : affected_scopes) {
            condition = condition ||
                    other_id != z3_ctx.int_val(value_decl->getID()) && z3::set_member(other_id, read_accesses(affected_scope, address));
        }
        ...
        if (result.has_value()) {
            ...
            std::cout << "var name: " << scope_ctx.get_var(id)->decl->getName().str() << std::endl;
        ...
        /*std::unordered_set<const class scope*> affected_scopes;
        for (auto cur_scope = scope; cur_scope != nullptr; cur_scope = cur_scope->parent) {
            affected_scopes.emplace(cur_scope);
        }
        ...
        for (const auto* affected_scope : affected_scopes) {
            condition = condition ||
                        other_id != z3_ctx.int_val(value_decl->getID()) && z3::set_member(other_id, read_restrict_writes(affected_scope, address));
        }
        const auto result = check(condition);
        if (result.has_value()) {
            const auto id = result.value().eval(z3_ctx.constant("ID", z3_ctx.int_sort())).as_int64();
    ...
    //for (const auto& [var_scope, vars] : restrict_writes) {
        /*auto var_scope = var->scope;
        while (var_scope && var_scope != scope) {
            var_scope = var_scope->parent;
        }
        if (!var_scope) {
            continue;
        }*/
   //     if (affected_scopes.count(var_scope)) {
   ...
   // const auto decl_id = z3_ctx.constant("DECL_ID", z3_ctx.int_sort());
    ...
    // const access_scope = scope_by_decl(value_decl);
    // for scope : restrict_writes_scopes
    //    all_decls = get_all_decls_in_scope
    //
    //
    //
...
namespace {
    std::string scoped(const std::string& meta, const scope* scope) {
        return meta + '#' + std::to_string(scope->id);
    }
}

z3::expr restrict_checker::read_accesses(const scope* scope, const z3::expr& address) {
    if (accessed_scopes.insert(scope).second) {
        const auto any_address = z3_ctx.constant("address", z3_ctx.int_sort());
        assume(z3::forall(any_address, read_accesses(scope, any_address) == z3::empty_set(z3_ctx.int_sort())));
    }
    return read_meta(scoped("ACCESSES", scope), address, z3_ctx.array_sort(z3_ctx.int_sort(), z3_ctx.bool_sort()));
}

void restrict_checker::write_access(const scope* scope, const z3::expr& address, const var* var) {
    write_meta(scoped("ACCESSES", scope), address, z3::set_add(read_accesses(scope, address), z3_ctx.int_val(var->decl->getID())));
}

z3::expr restrict_checker::read_restrict_writes(const scope* scope, const z3::expr& address) {
    if (restrict_written_scopes.insert(scope).second) {
        assume(read_meta(scoped("RESTRICT_WRITES", scope), z3_ctx.array_sort(z3_ctx.int_sort(), z3_ctx.bool_sort())) == z3::const_array(z3_ctx.int_sort(), z3::empty_set(z3_ctx.int_sort())));
        //const auto any_address = z3_ctx.constant("address", z3_ctx.int_sort());
        //const auto any_decl_id = z3_ctx.constant("decl_id", z3_ctx.int_sort());
        //assume(z3::forall(any_address, z3::forall(any_decl_id, !z3::set_member(any_decl_id, read_restrict_writes(scope, any_address)))));
    }
    return read_meta(scoped("RESTRICT_WRITES", scope), address, z3_ctx.array_sort(z3_ctx.int_sort(), z3_ctx.bool_sort()));
}

void restrict_checker::write_restrict_write(const scope* scope, const z3::expr& address, const var* var) {
    write_meta(scoped("RESTRICT_WRITES", scope), address, z3::set_add(read_restrict_writes(scope, address), z3_ctx.int_val(var->decl->getID())));
}